...
class MSCOCO_Kernel_Class(Dataset):
    def __init__(self, args, normalize=False):
        ...
        self.kernel_min = args.kernel_min
        self.kernel_max = args.kernel_max
        self.kernel_size_class = args.kernel_size_class

        self.class_range = np.linspace(self.kernel_min, self.kernel_max, self.kernel_size_class + 1)
    ...
    def __getitem__(self, n):
        ...
        kernel_class = np.zeros(self.kernel_size_class)
        for i in range(self.kernel_size_class):
            kernel_class[i] = np.where(((kernel_size >= kernel_size_class[i]) & (kernel_size < kernel_size_class[i+1])), 1, kernel_class)
        
        return {'Input': blur_image, 'GrandTruth': sharp_image, 'Kernel': kernel_image, 'Kernel_size': kernel_size, 'Kernel_class': kernel_class}
